...
class EnhancedBufferMemory(MemoryAdapter):
    ...
    def _detect_emotion(self, text: str) -> Optional[EmotionTag]:
        """Автоматическое определение эмоции в тексте без хардкода"""
        try:
            # Получаем эмоциональные маркеры из конфигурации
            emotion_markers = self.config.get('emotion_markers', {})
            
            text_lower = text.lower()
            
            # Проверяем каждую эмоцию из конфигурации
    ...
    def _calculate_importance(self, text: str, role: str) -> float:
        """Вычисляет важность сообщения без хардкода"""
        try:
            # Получаем настройки важности из конфигурации
            ...
            # Проверяем маркеры важности из конфигурации
    ...
    async def _trigger_summarization(self):
        """Запускает процесс суммаризации буфера"""
        try:
            if not self.llm or len(self.messages) < 3:
                return
            
            # Получаем сообщения для суммаризации (все кроме последних 2)
            ...
            if summary_text:
                ...
                # Добавляем в суммарную память
                self.summary_memory.append(summary_entry)
                
                # Ограничиваем количество суммарных записей
                if len(self.summary_memory) > 5:
                    self.summary_memory = self.summary_memory[-5:]
                
                # Очищаем буфер (оставляем только последние сообщения)
...
Резюме:"""
    ...
    def _calculate_summary_importance(self, messages: List[EnhancedMessage]) -> float:
        """Вычисляет важность суммарной записи"""
        if not messages:
            return 0.5
    ...
    def _extract_topics_single(self, text: str) -> List[str]:
        """Извлекает темы из одного сообщения без хардкода"""
        topics = set()
    ...
    def summarize_conversation(self, messages: List[Message]) -> str:
        ...
        if self.llm:
            try:
                ...
                return summary or self._simple_summarize(messages)
